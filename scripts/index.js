;(async () => {
  ...
  function animForecast() {
    ...
    forecastCheck.checked ? (forecastCheck.checked = false) : (forecastCheck.checked = true)
    ...
    if (photosData.errors === undefined) {
      photosData.forEach((pics, i) => {
        ...
        <img
          ...
          alt="${alt}"
          title="${alt}"
          class="object-cover max-h-xs border shadow-lg rounded-lg border-white dark:border-black"
        />
      `
        photosInject += html
      })
      ...
      photosBox.innerHTML = `<p class="shadow-md bg-gray-100 dark:text-white dark:bg-gray-900 text-gray-700 text-center self-start p-8 border rounded-lg">${city} has no photo's available 😔</p>`
    }
  }
